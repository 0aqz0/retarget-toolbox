...
class WuKongEnv(gym.Env):
    """docstring for WuKongEnv"""
    def __init__(self):
        super(WuKongEnv, self).__init__()
        self.pos = np.array([0,0,0.98])
    ...
    def step(self, action, custom_reward=None):
        ...
        p.resetBasePositionAndOrientation(self.Uid, self.pos, self.rot)
    ...
    def reset(self):
        ...
        self.Uid = p.loadURDF(os.path.join(os.path.dirname(os.path.realpath(__file__)),
            "assets/wk4_urdf/urdf/WuKongIV_preciseMass.urdf"), basePosition=self.pos, baseOrientation=self.rot, useFixedBase=False,
